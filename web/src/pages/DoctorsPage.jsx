...
export default function DoctorsPage() {
  ...
  const practice = ['ayurveda', 'dental'].includes(params.get('practice')) ? params.get('practice') : '';
  ...
  return (
    <>
      ...
      <div className="container-page py-12">
        ...
        <AsyncBlock query={q} empty={<EmptyState title="Doctor profiles coming soon" message="Our doctors’ profiles will be listed here shortly." />}>
          {(items) => <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">{items.map((d) => <DoctorCard key={d.id} doctor={d} />)}</div>}
        </AsyncBlock>
      </div>
    </>
  );
