...
const navClass = ({ isActive }) => `${linkBase} ${isActive ? 'bg-ink-100 text-ink-900' : 'text-ink-700 hover:bg-ink-50 hover:text-ink-900'}`;

const treatmentLinks = [
  { to: '/ayurveda/services', label: 'Ayurvedic treatments', hint: 'Shree Aadhitya Ayurvedic Hospital', dot: 'bg-ayur-600' },
  { to: '/dental/services', label: 'Dental treatments', hint: 'Shree Aadhitya Dental Hospital', dot: 'bg-dental-600' },
];
...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-ink-100 bg-white/95 backdrop-blur">
      <a href="#main" className="sr-only rounded bg-white px-3 py-2 focus:not-sr-only focus:absolute focus:left-4 focus:top-2">
        Skip to content
      </a>
      <div className="container-page flex h-16 items-center justify-between gap-4 lg:h-[72px]">
        ...
        <nav className="hidden items-center gap-1 lg:flex" aria-label="Main">
          ...
          <NavLink to="/ayurveda" end className={navClass}>Ayurveda</NavLink>
          <NavLink to="/dental" end className={navClass}>Dental</NavLink>

          <div className="group relative">
            <button type="button" className={`${linkBase} inline-flex items-center gap-1 text-ink-700 hover:bg-ink-50 hover:text-ink-900`} aria-haspopup="true">
            ...
            <div className="invisible absolute left-0 top-full w-72 pt-2 opacity-0 transition group-focus-within:visible group-focus-within:opacity-100 group-hover:visible group-hover:opacity-100">
              <ul className="rounded-lg border border-ink-100 bg-white p-2 shadow-lg">
                {treatmentLinks.map((l) => (
                  <li key={l.to}>
                    <Link to={l.to} className="flex items-start gap-3 rounded-md p-3 hover:bg-ink-50">
                      <span className={`mt-1.5 size-2 shrink-0 rounded-full ${l.dot}`} aria-hidden="true" />
                      <span>
      ...
      {open && (
        <nav id="mobile-nav" aria-label="Mobile" className="fixed inset-x-0 top-16 bottom-0 overflow-y-auto border-t border-ink-100 bg-white lg:hidden">
          <ul className="container-page space-y-1 py-4">
            {[
              ['/', 'Home', true],
              ['/ayurveda', 'Ayurveda'],
              ['/dental', 'Dental'],
              ['/ayurveda/services', 'Ayurvedic treatments'],
              ...
              <li key={to}>
                <NavLink to={to} end={!!end || to === '/ayurveda' || to === '/dental'} className={({ isActive }) => `block rounded-md px-3 py-3 text-base font-medium ${isActive ? 'bg-ink-100 text-ink-900' : 'text-ink-700'}`}>
                  {label}
                </NavLink>
              </li>
            ))}
