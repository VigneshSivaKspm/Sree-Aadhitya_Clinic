  ...
  if (compact) {
    return (
      ...
      >
        ...
        <ArrowRight
          className={`size-5 shrink-0 ${t.text} transition-transform group-hover:translate-x-0.5`}
          aria-hidden="true"
        />
      </Link>
    );
    ...
    >
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <div className="mt-4 flex items-center justify-between text-sm">
          ...
          <span
            className={`inline-flex items-center gap-1 font-medium ${t.text}`}
          >
            Learn more{" "}
            <ArrowRight
...
export function DoctorCard({ doctor }) {
  ...
  return (
    <article className="flex h-full flex-col overflow-hidden rounded-xl border border-ink-100 bg-white">
      ...
      <div className="flex flex-1 flex-col p-5">
        <span
          className={`mb-2 w-fit rounded-full px-2.5 py-0.5 text-xs font-medium ${t.chip}`}
        >
        ...
        <p className={`mt-1 text-sm font-medium ${t.text}`}>
          {doctor.specialization}
        </p>
        {doctor.experience && (
          <p className="mt-1 text-sm text-ink-500">{doctor.experience}</p>
        )}
        {doctor.bio && (
          <p className="mt-3 text-sm leading-relaxed text-ink-600">
            {truncate(doctor.bio, 140)}
          </p>
        )}
      </div>
    </article>
  ...
  return (
    <figure className="flex h-full flex-col rounded-xl border border-ink-100 bg-white p-6">
      ...
      {testimonial.rating > 0 && (
        <div
          className="mt-4 flex gap-0.5 text-gold-500"
          role="img"
        ...
        >
          {Array.from({ length: 5 }, (_, i) => (
            <Star
              key={i}
              className={`size-4 ${i < testimonial.rating ? "fill-current" : "text-ink-200"}`}
              aria-hidden="true"
            />
          ))}
        </div>
      ...
      <figcaption className="mt-4 text-sm font-medium text-ink-900">
        {testimonial.name}
        {testimonial.isDemo && (
          <span className="ml-2 rounded bg-gold-500/15 px-1.5 py-0.5 text-xs font-medium text-gold-600">
            Sample
          </span>
        )}
      </figcaption>
