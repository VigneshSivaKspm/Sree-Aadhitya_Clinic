...
export function ErrorState({
  ...
  return (
    <div
      role="alert"
      className="rounded-xl border border-danger-600/20 bg-danger-50 px-6 py-10 text-center"
    >
...
export function DemoNotice({ show = true }) {
  ...
  return (
    <p className="mb-6 rounded-md border border-gold-500/30 bg-gold-500/10 px-4 py-2.5 text-sm text-ink-800">
      <strong className="font-semibold">Sample content.</strong> This section is
      showing placeholder content until real details are added from the admin
      panel.
    </p>
  );
}
